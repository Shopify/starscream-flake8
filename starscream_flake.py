...
CLASS_DEFINITION = re.compile(r'\s*class\s')
...
def detect_missing_blank_line_after_class_definition(previous_logical, blank_lines):
    '''
    ...
    if match and blank_lines != 1:
        return match.start(), 'SC04 expected 1 blank lines after a class definition, found %d' % blank_lines
...
def main(logical_line, physical_line, line_number, filename, previous_logical, blank_lines):
    ...
    result = detect_missing_blank_line_after_class_definition(previous_logical, blank_lines)
    if result:
        results.append(result)
